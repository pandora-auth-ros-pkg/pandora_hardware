...
void process_packet() 
{
  ...
  int speed_left = (int)packet_buffer[4] + 10*(int)packet_buffer[3] + 100*(int)packet_buffer[2];
  int speed_right = (int)packet_buffer[9] + 10*(int)packet_buffer[8] + 100*(int)packet_buffer[7];
...
void check_for_new_bytes_received()
{
  while(serial_get_received_bytes(USB_COMM) != receive_buffer_position)
  {
    ...
    if (receive_buffer[receive_buffer_position] == '$' && !recording_packet)
    {
    ...
    if (recording_packet)
    {
      packet_buffer[packet_buffer_position++] = receive_buffer[receive_buffer_position];
      increment_receive_buffer_position();
